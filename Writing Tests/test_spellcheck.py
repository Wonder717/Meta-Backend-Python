...
def test_length(input_value):
    ...
    assert spellcheck.word_count(sentence = input_value) < 10

    assert spellcheck.char_count(sentence = input_value) < 50 
...
def test_struc(input_value):
    ...
    """
    ### WRITE SOLUTION CODE HERE
    assert spellcheck.first_char(sentence=input_value).isupper() == True 
    assert spellcheck.last_char(sentence= input_value) == "."
